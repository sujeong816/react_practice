...
const StudentEdit = React.memo(({ dispatch }: Props) => { 
    ...
    useEffect(() => { if (id) load(); }, []) // 컴포넌트가 마운트 될 때, id 값으로 학생을 조회한다 
    ...
    return ( 
        <Div> 
            ...
            <div> 
                <label> 
                    <input type="radio" name="sex" value="남" onChange={onChange} 
                            checked={student.sex === "남"} /> 
                    <span>남자</span>
                </label> 
                <label> 
                    <input type="radio" name="sex" value="여" onChange={onChange} 
